...
function markGroup(o, s, c, b) {
    ...
    if (b == " Add ")
    {
        ...
        else if (s=='x_QRuleList')
    	{
            ...
            {            	
            	alert('请输入一数值介于 0 to ' + document.form.qos_maxbw_x_0.value + '.');	
        ...
        else if (s=='x_UQRuleList')
    	{
            ...
            {            	
            	alert('请输入一数值介于 0 to ' + document.form.qos_umaxbw_x_0.value + '.');
...
function load_wizard()
{
   ...
   {
       rst = confirm("感谢您购买「华硕无线路由器」，您要直接启动「快速设置」程序吗？");
       if (rst)        
       {       	   
...
function onSubmit()
{
   ...
   else if (document.form.current_page.value == "Advanced_LANWAN_Content.asp")
   {
        if (document.form.wan_proto.value == "Static IP")
  	{
  	    ...
  	    {    
  	       alert("当您将IP或子网掩码栏目保留空白，连接类型即会设置为「自动获取IP」!");
...
function onSubmitCtrlOnly(o, s)
{
   ...
   if (s=='Upload1') 
   {
   	if (varload) 
   	{
   	   alert("It spends about 30 seconds to upload and program the firmware. Please do not press '上传' twice.");
   	   return false;
   	}  
...
{   
   ...
   {      	   	
   	...
   	if (action.value == "Set")
   	{
   	   ...
   	   {
   		alert("当您将IP或子网掩码栏目保留空白，连接类型即会设置为「自动获取IP」!1");	
   	   	return false;	
   	   }	   	     	   
   	}
...
function change_common(o, s, v)
{
  ...
  {
  	if (o.value == "DROP")
           alert("选择「丢弃」时，会将全部WAN to LAN封包当中除了与过滤表内相符以外的部份丢弃。请谨慎使用。");
  }
  else if (s == "FirewallConfig" && v=="LanWanDefaultAct")
  {
  	if (o.value == "DROP")
           alert("选择「丢弃」时，会将全部LAN to WAN封包当中除了与过滤表内相符以外的部份丢弃。请谨慎使用。");        
  }   
...
function change_common_radio(o, s, v, r)
{
  ...
  else if (v=="x_AESEnable")
  {
  	if (r == '1')
  	{
  	   ...
  	   {
  		rst = confirm("如果您要启用AES unique功能，认证方式将会因此设成「系统开放」!");
 		...
 		if (rst)  		  		   		 		   
  		...
  		else
  		{
           ...
           else
           {
           	...
           	{
           	    rst = confirm("如果您要启用AES unique功能，应该也要启用WEP!");
...
{   	      	   	   	
   if (v == "wl_wpa_psk")
   {
   	...
   	{   	   
   	   if (o.value.length < 8)
   	   {	
   	      alert("WPA密钥不得超出7个字符!!!如果您将本栏目保留空白，系统将会指定00000000作为您的密码。");
...
function checkSubnet()
{
        ...
        {   
            if (confirm("DHCP服务器的设置与目前LAN的IP地址不符。您要采用自动的方式变更吗？"))
            {	        	         	        
...
{                                                     
        ...
        {   
            if (confirm("DHCP服务器的设置与目前「无线防火墙」的IP地址不符。您要采用自动的方式变更吗？"))
            {	        	         	        
