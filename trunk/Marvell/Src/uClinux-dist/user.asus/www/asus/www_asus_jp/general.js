...
function markGroup(o, s, c, b) {
    ...
    if (b == " Add ")
    {
        ...
        else if (s=='x_QRuleList')
    	{
            ...
            {            	
            	alert('以下の間の値を入力してください 0 to ' + document.form.qos_maxbw_x_0.value + '.');	
        ...
        else if (s=='x_UQRuleList')
    	{
            ...
            {            	
            	alert('以下の間の値を入力してください 0 to ' + document.form.qos_umaxbw_x_0.value + '.');
    ...
    if (bFlag==1)
        alert("フィールドはブランクにできませｎ!!!"); 
    else if(cFlag==1)
        alert("この表は許可します " + c + " アイテム!!!");
    else 
    {    	
       if (s=='WLFilterList')
       {           	        	                                 
  ...
  if(o.value != "") 
  {
    if(o.value.length == 12)      
    {
       for(i=0; i<o.value.length; i++) 
       {
          ...
          {
             alert('12の16進数文字を含む、完全なMACアドレスを入力してください。');
             o.value = "";
             o.focus();	
             return false;
          }   
...
{      	
   ...
   {
      alert('以下の間の値を入力してください ' + min + ' to ' + max + '.');
      o.value = min;
      o.focus();
      return false;    
   }
...
function validate_ipaddr(o, v) 
{
  ...
  for(i=0; i<o.value.length; i++) {
     ...
     {           
        if ( num<0 || num>255 || c!='.') 
        {
            alert(o.value + ' 有効なIPアドレスではありません!');
            o.value = "";
            o.focus();
            return false;
        }    
...
function load_wizard()
{
   ...
   {
       rst = confirm("ASUSワイヤレスルータをお買上げいただきましたありがとうございます。クイックセットアップを直接起動しますか?");
       if (rst)        
       {       	   
...
function onSubmit()
{
   ...
   else if (document.form.current_page.value == "Advanced_LANWAN_Content.asp")
   {
        if (document.form.wan_proto.value == "Static IP")
  	{
  	    ...
  	    {    
  	       alert("IPまたはサブネットマスクフィールドをブランクにしておくと、接続タイプは自動IPとして設定されます!");
...
function onSubmitCtrlOnly(o, s)
{
   ...
   if (s=='Upload1') 
   {
   	if (varload) 
   	{
   	   alert("It spends about 30 seconds to upload and program the firmware. Please do not press 'Upload' twice.");
   	   return false;
   	}  
...
{   
   ...
   {      	   	
   	...
   	if (action.value == "Set")
   	{
   	   ...
   	   {
   		alert("IPまたはサブネットマスクフィールドをブランクにしておくと、接続タイプは自動IPとして設定されます!1");	
   	   	return false;	
   	   }	   	     	   
   	}
...
function change_common(o, s, v)
{
  ...
  {
  	if (o.value == "DROP")
           alert("DROPを選択すると、フィルタテーブルに一致しないWAN対LANパケットがドロップします。使用に際しては、注意してください。");
  }
...
function change_common_radio(o, s, v, r)
{
  ...
  else if (v=="x_AESEnable")
  {
  	if (r == '1')
  	{
  	   ...
  	   {
  		rst = confirm("ＡＥＳ独特の機能を有効になる場合は、オープンシステムど同じの認証を設定してください。");
 		...
 		if (rst)  		  		   		 		   
  		...
  		else
  		{
           ...
           else
           {
           	...
           	{
           	    rst = confirm("ＡＥＳ独特の機能を有効になる場合は、ＷＥＰも有効になってください。");
...
{ 
  ...
  alert('完全なWEPキーを入力してください。'); 
  return false;  
}

/* Factory Reset Warning */
function confirmRestore(){
	if(confirm('WARNING:\nAll your settings will be lost!\nProceed or not?注意：すべての設定は失ってしまいます。続けますか。')) {		
...
{   	      	   	   	
   if (v == "wl_wpa_psk")
   {
   	...
   	{   	   
   	   if (o.value.length < 8)
   	   {	
   	      alert("パスフレーズは7文字以上にする必要があります!!! このフィールドをブランクにしておくと、システムはパスフレーズとして00000000を割り当てます。");
...
function checkSubnet()
{
        ...
        {   
            if (confirm("ＤＨＣＰサーバーの設定はＬＡＮのＩＰアドレスと一致していません。自動的に変更しますか。"))
            {	        	         	        
...
{                                                     
        ...
        {   
            if (confirm("ＤＨＣＰサーバーの設定はワイヤレスファイヤフォールのＩＰアドレスと一致していません。自動的に変更しますか。"))
            {	        	         	        
