...
function markGroup(o, s, c, b) {
    ...
    if (b == " Add ")
    {
        ...
        else if (s=='x_QRuleList')
    	{
            ...
            {            	
            	alert('請輸入一數值介於 0 to ' + document.form.qos_maxbw_x_0.value + '.');	
        ...
        else if (s=='x_UQRuleList')
    	{
            ...
            {            	
            	alert('請輸入一數值介於 0 to ' + document.form.qos_umaxbw_x_0.value + '.');
...
function load_wizard()
{
   ...
   {
       rst = confirm("感謝您購買「華碩無線路由器」，您要直接啟動「快速設定」程式嗎？");
       if (rst)        
       {       	   
...
function onSubmit()
{
   ...
   else if (document.form.current_page.value == "Advanced_LANWAN_Content.asp")
   {
        if (document.form.wan_proto.value == "Static IP")
  	{
  	    ...
  	    {    
  	       alert("當您將IP或子網路遮罩欄位保留空白，連線類型即會設定為「自動IP」!");
...
{   
   ...
   {      	   	
   	...
   	if (action.value == "Set")
   	{
   	   ...
   	   {
   		alert("當您將IP或子網路遮罩欄位保留空白，連線類型即會設定為「自動IP」!1");	
   	   	return false;	
   	   }	   	     	   
   	}
...
function change_common(o, s, v)
{
  ...
  {
  	if (o.value == "DROP")
           alert("選擇「丟棄」時，會將全部WAN to LAN封包當中除了與過濾表內相符以外的部份丟棄。請謹慎使用。");
  }
  else if (s == "FirewallConfig" && v=="LanWanDefaultAct")
  {
  	if (o.value == "DROP")
           alert("選擇「丟棄」時，會將全部LAN to WAN封包當中除了與過濾表內相符以外的部份丟棄。請謹慎使用。");        
  }   
...
function change_common_radio(o, s, v, r)
{
  ...
  else if (v=="x_AESEnable")
  {
  	if (r == '1')
  	{
  	   ...
  	   {
  		rst = confirm("如果您要啟用AES unique功能，驗證方式將會因此設成「系統開放」!");
 		...
 		if (rst)  		  		   		 		   
  		...
  		else
  		{
           ...
           else
           {
           	...
           	{
           	    rst = confirm("如果您要啟用AES unique功能，應該也要啟用WEP!");
...
{   	      	   	   	
   if (v == "wl_wpa_psk")
   {
   	...
   	{   	   
   	   if (o.value.length < 8)
   	   {	
   	      alert("WPA金鑰不得超出7個字元!!!如果您將本欄位保留空白，系統將會指定00000000作為您的通關密語。");
...
function checkSubnet()
{
        ...
        {   
            if (confirm("DHCP伺服器的設定值與目前LAN的IP位址不符。您要採用自動的方式變更嗎？"))
            {	        	         	        
...
{                                                     
        ...
        {   
            if (confirm("DHCP伺服器的設定值與目前「無線防火牆」的IP位址不符。您要採用自動的方式變更嗎？"))
            {	        	         	        
