...
{
	dprintf("launching /etc/init.d/rcS\n");
	eval("sh","/etc/init.d/rcS");
	dprintf("done\n");
	return 0;
}
...
{
	int ret;
	char *httpd_argv[] = {"httpd", "WAN", NULL};
	pid_t pid;
	...
	ret = _eval(httpd_argv, NULL, 0, &pid);
#else
	ret = eval("httpd");
#endif
...
start_upnp(void)
{
	...
	if (ret != 0) {
	    ...
	    wan_ifname = ( nvram_match(strcat_r(prefix, "proto", var), "pppoe") || nvram_match(strcat_r(prefix, "proto", var), "pptp")) ? 
					nvram_safe_get(strcat_r(prefix, "pppoe_ifname", var)) :
					nvram_safe_get(strcat_r(prefix, "ifname", var));
#ifdef ASUS_EXT
	    upnp_argv[2] = nvram_safe_get("lan_ifname");
	    upnp_argv[4] = wan_ifname;
	    ret = _eval(upnp_argv, NULL, 0, &pid);
#else
	    ret = eval("upnp", "-D",
