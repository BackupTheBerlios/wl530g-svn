...
start_firewall2(char *wan_ifname)
{
   	...
   	if (nvram_match("router_disable", "0") && nvram_invmatch("sp_battle_ips", "0"))
	{
		...
		sprintf(cmd, "iptables -t nat -I POSTROUTING -p udp -s %s --dport %s -j NETMAP --to %s", lan_class, BASEPORT, wan_ip);
		system(cmd);
		...
		eval("iptables", "-I", "FORWARD", "-p", "udp", "--dport", BASEPORT, "-j", "ACCEPT");
	}
	dprintf("done\n");
	return 0;
}
