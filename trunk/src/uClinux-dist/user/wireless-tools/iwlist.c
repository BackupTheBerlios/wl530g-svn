...
static const struct iwmask_name iw_cipher_name[] = {
...
#define	IW_CIPHER_NUM		IW_ARRAY_LEN(iw_cipher_name)
...
static const struct iwmask_name iw_key_mgmt_name[] = {
  { IW_AUTH_KEY_MGMT_802_1X,	"802.1x" },
  { IW_AUTH_KEY_MGMT_PSK,	"PSK" },
};
#define	IW_KEY_MGMT_NUM		IW_ARRAY_LEN(iw_key_mgmt_name)
...
static const struct iw_auth_descr	iw_auth_settings[] = {
  ...
  { IW_AUTH_KEY_MGMT, "Key management", iw_key_mgmt_name, IW_KEY_MGMT_NUM },
  { IW_AUTH_CIPHER_PAIRWISE, "Pairwise cipher", iw_cipher_name, IW_CIPHER_NUM },
  { IW_AUTH_CIPHER_GROUP, "Pairwise cipher", iw_cipher_name, IW_CIPHER_NUM },
		...
		int		buflen)
{
  ...
  if(memcmp(&iebuf[offset], wpa_oui, 3) != 0)
    ...
    {
      printf("                        Group Cipher :");
      iw_print_mask_name(iebuf[offset+3],
			 iw_cipher_name, IW_CIPHER_NUM, " ");
      printf("\n");
    }
  offset += 4;
  ...
  for(i = 0; i < cnt; i++)
	...
	{
	  iw_print_mask_name(iebuf[offset+3],
			     iw_key_mgmt_name, IW_KEY_MGMT_NUM, " ");
 	}
       offset+=4;
     }
  printf("\n");
	      ...
	      if(iw_get_ext(skfd, ifname, SIOCGIWAUTH, &wrq) >= 0)
		{
		  ...
		  if(iw_auth_settings[k].names != NULL)
		    iw_print_mask_name(wrq.u.param.value,
				       iw_wpa_ver_name, IW_WPA_VER_NUM,
				       "\n\t\t");
		  else
      ...
      for(k = 1; k <= range.max_encoding_tokens; k++)
	{
	  ...
	  if((wrq.u.data.flags & IW_ENCODE_DISABLED) ||
	  ...
	  else
	    {
	      ...
	      printf("\t\t     Algorithm: ");
	      if(extinfo->alg >= IW_ENCODE_ALG_NUM)
		printf("unknown (%d)\n", extinfo->alg);
	      else
		printf("%s", iw_encode_alg_name[extinfo->alg]);
...
static void iw_usage(int status)
{
  ...
  fprintf(f,   "Usage: iwlist [interface] %s %s\n", iwlist_cmds[0].cmd,
